...
from Dataset import *
import os
from models import resnet_ovo, resnet_96x96_ovo,OvO_Loss_CE
from Utils import *
from fit import *
from Models import *
...
def main(args):
    ...
    num_classes_ovo = int(args.n_classes * (args.n_classes - 1) / 2)
    ...
    model_ovo = resnet(num_classes=num_classes_ovo, depth=110)
    ...
    criterion_ovo = OvO_Loss_CE.OvOLoss(num_classes = num_classes_ovo)
    ...
    training_obg = BaseClassifier(args, model = model_ovo, optimizer = optimizer_ovo, criterion = criterion_ovo,
                                  scheduler = scheduler, train_dl = loaders['Train_load'], valid_dl = loaders['Valid_load'],
                                  test_dl = loaders['Test_load'], device = device, transform_matrix = trans_matrix, state=  state,
                                  model2=None)
